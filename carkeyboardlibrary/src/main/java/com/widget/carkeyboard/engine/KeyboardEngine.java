...
import com.widget.carkeyboard.neighbor.NeighborManager;
...
public class KeyboardEngine {
    ...
    private FuncKeyTransformer funcKeyTransformer;
    //确定键在未输完时能否被启用
    private boolean okEnabled;

    public KeyboardEngine() {
        funcKeyTransformer = new FuncKeyTransformer();
        // keys
        mLayoutMixer.addKeyTransformer(funcKeyTransformer);
    ...
    public void setOkEnabled(boolean okEnabled) {
        if (funcKeyTransformer != null) {
            funcKeyTransformer.setOkEnabled(okEnabled);
        }
    }
    ...
    public KeyboardEntry update(String presetNumber, int selectCharIndex, boolean showMoreLayout, NumberType fixedNumberType) {
        ...
        Log.e("detectNumberType",detectNumberType.toString());
        // 车牌限制长度
